...
describe('FilterPanel — date range', () => {
  ...
  it('shows a "From" chip when a start date is set', () => {
    renderPanel()
    const fromLabel = screen.getByText('From')
    fireEvent.change(fromLabel.querySelector('input'), { target: { value: '2023-01-01' } })
    // "From" now appears as both the date label and a chip-kind span
  ...
  it('shows a "To" chip when an end date is set', () => {
    renderPanel()
    const toLabel = screen.getByText('To')
    fireEvent.change(toLabel.querySelector('input'), { target: { value: '2023-12-31' } })
